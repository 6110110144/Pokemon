...
public class GameMenu extends GameScreen{
    ...
    private JButton selectButton,itemComboBoxButton,battlePokeballButton,battleElixirButton,battleAttackButton,battleSwitchButton,battleNextButton,closeButton;
    ...
    private ActionListener returnItemAction,battlePokeballAction,battleElixirAction,battleAttackAction,battleSwitchAction,battleComboboxAction,closeAction;
    ...
    public GameMenu(Container c,String pokemonSpecies,String playerName,String pokemonName){
        ...
        itemPokemonUseAction = new ActionListener(){
            public void actionPerformed(ActionEvent e){
                switch ((String)itemComboBox.getSelectedItem()){
                    case "HP Booster" : 
                        if(items.getHPBoosterAmount() <= 0){
                            mainTextArea.setText("You don't have a HP Booster!");
                        }
                        else{
                            items.useHPBooster(reserve);
                            mainTextArea.setText("Your "+reserve.getname()+" max HP has rose to "+reserve.getMaxHP()+"!!!");
                        }
                        break;
                    case "ATK Booster" : 
                        if(items.getATKBoosterAmount() <= 0){
                            mainTextArea.setText("You don't have a ATK Booster!");
                        }
                        else{
                            items.useATKBooster(reserve);
                            mainTextArea.setText("Your "+reserve.getname()+" ATK has rose to "+reserve.getatk()+"!!!");
                        }
                        break;
                    case "DEF Booster" : 
                        if(items.getDEFBoosterAmount() <= 0){
                            mainTextArea.setText("You don't have a DEF Booster!");
                        }
                        else{
                            items.useDEFBooster(reserve);
                            mainTextArea.setText("Your "+reserve.getname()+" DEF has rose to "+reserve.getdef()+"!!!");
                        }
                        break;                          
                    case "SPDBooster" : 
                        if(items.getSPDBoosteramount() <= 0){
                            mainTextArea.setText("You don't have a SPDBooster!");
                        }
                        else{
                        ...
                        break;
                    case "Elixir" :
                        if(items.getElixirAmount() <= 0){
                            mainTextArea.setText("You don't have a Elixir!");
                        }
                        else{
                            items.useElixir(reserve);
                            mainTextArea.setText("Your "+reserve.getname()+" HP has been restored to "+reserve.getCurrentHP()+"/"+reserve.getMaxHP()+"!");
                        }
                        break;
                    default :
            ...
            public void actionPerformed(ActionEvent e){
                for(i=0;i<bags.size();i++){
                    if(bags.get(i).getname().equals((String)selectComboBox.getSelectedItem())){
                        ...
                        "- HP Booster ["+items.getHPBoosterAmount()+"] use for increase max HP"+
                        "\n- ATK Booster ["+items.getATKBoosterAmount()+"] use for increase ATK"+
                        "\n- DEF Booster ["+items.getDEFBoosterAmount()+"] use for increase DEF"+
                        "\n- SPDBooster ["+items.getSPDBoosteramount()+"] use for increase SPD"+
                        "\n- Elixir ["+items.getElixirAmount()+"] use to restore MaxHP " +
            ...
            public void actionPerformed(ActionEvent e){
                ...
                for(i=0;i<bags.size();i++){
                    if(bags.get(i).getname().equals((String)selectComboBox.getSelectedItem())){
                        ...
                        "\nHP         : "+bags.get(i).getCurrentHP()+"/"+bags.get(i).getMaxHP()+
        ...
        restComboAction = new ActionListener(){
            public void actionPerformed(ActionEvent e){
                ...
                for(i=0;i<bags.size();i++){
                    if(bags.get(i).getname().equals((String)selectComboBox.getSelectedItem())){
                        bags.get(i).CurrentHPChange(bags.get(i).getMaxHP()); //heal to full hp
                        mainTextArea.setText(bags.get(i).getname()+"'s HP is restored to "+bags.get(i).getCurrentHP()+"!!!");
                    }
                }
                returnButton.setVisible(true);
                returnButton.addActionListener(returnStatAction);
        ...
        battleElixirButton = new JButton("Use Elixr");
        battleElixirButton.setBounds(300,500,400,50);
        battleElixirButton.setFont(normalFont);
        battleElixirButton.setBackground(Color.black);
        battleElixirButton.setForeground(Color.white);
        battleElixirButton.setVisible(false);
        battleElixirAction = new ActionListener(){
            public void actionPerformed(ActionEvent e){
                if(items.getElixirAmount()<=0){
                    mainTextArea.setText("You don't have an Elixir!");
                }   
                else{
                    items.useElixir(reserve);
                    RemoveNextActionListener();
                    mainTextArea.setText("You've use an Elixr!"+"\n"+reserve.getname()+" HP is restored to "+reserve.getCurrentHP()+"/"+reserve.getMaxHP()+"!");            
                }
                battleNextButton.addActionListener(battleAttackNextMenuAction);
            }
        };
        battleElixirButton.addActionListener(battleElixirAction);
        c.add(battleElixirButton);
        ...
        inputNameAction = new ActionListener(){
            public void actionPerformed(ActionEvent e){
                ...
                inputNameTextField.setVisible(false);
                mainTextArea.setText(wildPokemon.getname()+" is your Pokemon now!");
                    ...
                    if(bags.get(i).getname().equals((String)selectComboBox.getSelectedItem())){
                        ...
                        switch(numreserve){
                            ...
                            case 1:
                                ...
                                break;
                            case 2:
                                wildPokemon.lvlchange(reserve.getlvl()); 
                                break;
                            case 3:
                                wildPokemon.lvlchange(reserve.getlvl() + 1);   
                                break;
                        ...
                        + reserve.getname() + " HP : "+ reserve.getCurrentHP() + "/" + reserve.getMaxHP() + "   LVL : " + reserve.getlvl() +
                        "\n\nWild " + wildPokemon.getspecies() + " HP : " + wildPokemon.getCurrentHP() + "/" + wildPokemon.getMaxHP() + "   LVL : " + wildPokemon.getlvl()
                        );
    ...
    private void wildAttack(){
        ...
        //if not yet
            ...
            mainTextArea.setText(
                ...
                +"\n\n" + reserve.getname() + " HP is " + reserve.getCurrentHP()+"/" + reserve.getMaxHP()+" now!"
            );
            RemoveNextActionListener();
            if(turn < 1){
    ...
    private void playerAttack(){
        ...
        //if not win yet
            ...
            mainTextArea.setText(
                ...
                +"\n\n" + wildPokemon.getspecies() + " HP is " + wildPokemon.getCurrentHP() + "/" + wildPokemon.getMaxHP() + " now!\n"
            );
            RemoveNextActionListener();
            if(turn < 1){
    ...
    private void TurnOnBattleMenu(){
        mainTextArea.setText(reserve.getname() + " HP : "+ reserve.getCurrentHP() + "/" + reserve.getMaxHP() + "   LVL : " + reserve.getlvl() +
        "\n\nWild " + wildPokemon.getspecies() + " HP : " + wildPokemon.getCurrentHP() + "/" + wildPokemon.getMaxHP() + "   LVL : " + wildPokemon.getlvl()
        );
        battleAttackButton.setVisible(true);
        battleElixirButton.setVisible(true);
